...
function requestToralRows() {
    ...
    $.ajax({
        url: 'ajax/get_active_user_loans.php',
        type: 'POST',
        data: {
            "genre": genre,
            "library": library,
...
function returnBook(loanID, bookID) {
    ...
    if (confirm('Are you sure you want to return the book with id \"' + bookID + '\"?')) {
        $.ajax({
...
function createTable(table, rows, data) {
    ...
    for (let book in data.books) {
        ...
        cell3.innerHTML = data.books[book].days_left;
        cell4.innerHTML = data.books[book].due_date;
        ...
        cell6.innerHTML = '' +
            ...
            'onclick="returnBook(' + data.books[book].book_loan_id + ', ' + data.books[book].id + ')">' +
            '<img src="images/book_return.svg" width="24px">' +
            '</button>';
