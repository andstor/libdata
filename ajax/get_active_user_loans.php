...
if (isset($_POST['pn'])) {
    ...
    $query = "SELECT DISTINCT
                  ...
                  p.publisher,
                  lb.name AS library,
                  (SELECT DATEDIFF(l2.due_date, CURRENT_TIMESTAMP)) AS days_left,
                  l2.due_date
                FROM (SELECT DISTINCT bd2.*
                  ...
                  WHERE l2.user_id = '$user_id' AND l2.book_loan_id IN (
                      SELECT bl.book_loan_id
    ...
    if ($book !== false) {
        ...
        foreach ($data["books"] AS $key => $value) {
            if ($value["due_date"] == '0000-00-00 00:00:00') {
                $data["books"][$key]["due_date"] = "";
            } else {
                $data["books"][$key]["due_date"] = formatDateTime($value["due_date"]);
            }
        }
    }
