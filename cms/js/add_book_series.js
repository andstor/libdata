...
function requestToralRows() {
    ...
    $.ajax({
        url: 'ajax/get_book_descriptions.php',
        type: 'POST',
        data: {
            "genre": genre,
            "search": search
...
function addToList(isbn) {
    for (let book in lastData.books) {
        if (lastData.books[book].isbn == isbn) {
            ...
            if (!alreadyExists) {
                books.push(lastData.books[book]);
                updateSelectedBooks(lastData.books[book]);
            }
        }
    }
}
...
function deleteBookField(elemId, bookISBN) {
    for (let i = 0; i < books.length; i++) {
        if(books[i].isbn == bookISBN) books.splice(i,1)
    }
    $('#' + elemId).remove();
}

function updateSelectedBooks(book) {
    let booksContainer = document.getElementById('books-container');
    let $booksContainer = $(booksContainer);

    let deleteVar = '\'bookInputGroup' + numBooks + "\'";
    // language=HTML
    let bookInputsTemplate = ' <div class="form-inline" id="bookInputGroup' + numBooks + '">' +
        '                                <div class="form-group mb-2">' +
        '                                        <input type="text" id="inputbook" class="form-control" name="book_isbn[]"' +
        '                                               value="' + book.isbn + '" readonly>' +
        '                                    </div>' +
        '                                <div class="form-group mx-sm-3 mb-2">' +
        '                                        <input type="text" id="inputbookLastName" class="form-control"' +
        ...
        '                                </div>' +
        '                                  <button type="button" onclick="deleteBookField(' + deleteVar + ',' + book.isbn + ');" class="btn btn-outline-danger mb-2">X</button>' +
        '                         </div>';

    $booksContainer.append(bookInputsTemplate);
    numBooks++;
}
...
function createTable(table, rows, data) {
    ...
    for (let book in data.books) {
        ...
        cell0.innerHTML = data.books[book].isbn;
        cell1.innerHTML = data.books[book].title;
        cell2.innerHTML = data.books[book].language;
        cell3.innerHTML = data.books[book].publisher;


        cell4.innerHTML = '' +
            '<!-- Delete book -->' +
            '<button type="button" class="btn btn-success" onclick="addToList(' + data.books[book].isbn + ')">' +
            '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>' +
            '</button>';

        cell4.setAttribute("id", "text-center");
