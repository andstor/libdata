...
function deleteWaitingList(id) {
    console.log("deleting...");

    if (confirm('Are you sure you want to delete the waiting list with id \"' + id + '\" from the database?')) {
        $.ajax({
            url: 'ajax/waiting_list_delete.php',
            type: 'POST',
            data: {
                "delete_id": id,
            },
...
function createTable(table, rows, data) {
    ...
    let i = 0;
    for (let waitingList in data.waiting_list) {
        let row = tableRef.insertRow(-1);

        for (let j = 0; j <= numCells; ++j) {
            window["cell" + j] = row.insertCell(j);
        }
        cell0.innerHTML = data.waiting_list[waitingList].waiting_list_id;
        cell1.innerHTML = data.waiting_list[waitingList].isbn;
        cell2.innerHTML = data.waiting_list[waitingList].title;
        cell3.innerHTML = data.waiting_list[waitingList].num_users;
        cell4.innerHTML = data.waiting_list[waitingList].name;

        cell5.innerHTML = '' +
            '<button type="button" class="btn btn-success btn-circle"' +
            'onclick="location.href=\'view_waiting_list.php?list_id=' + data.waiting_list[waitingList].waiting_list_id + '\'">' +
            '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>' +
            '</button>' +

            '<!-- Delete book -->' +
            '<button type="button" class="btn btn-danger btn-circle" onclick="deleteWaitingList(' + data.waiting_list[waitingList].waiting_list_id + ')">' +
